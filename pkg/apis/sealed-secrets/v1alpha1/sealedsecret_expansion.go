...
func NewSealedSecret(codecs runtimeserializer.CodecFactory, pubKey *rsa.PublicKey, secret *v1.Secret, sessionKeySeed string) (*SealedSecret, error) {
	...
	sessionKeyReader, err := crypto.GenerateSessionKeyReader(sessionKeySeed, []byte(secret.String()))
	if (err != nil){
		return nil, err
	}

	for key, value := range secret.Data {
		ciphertext, err := crypto.HybridEncrypt(sessionKeyReader, pubKey, value, label)
		if err != nil {
			return nil, err
		}
	...
	for key, value := range secret.StringData {
		ciphertext, err := crypto.HybridEncrypt(sessionKeyReader, pubKey, []byte(value), label)
		if err != nil {
			return nil, err
		}
		s.Spec.EncryptedData[key] = base64.StdEncoding.EncodeToString(ciphertext)
